...
def createReport(report_data, analysis):
    ...
    doc.add_heading(report_data['title'], 1) if report_data['title'] else doc.add_heading( 'Reporte Preliminar de Calidad del Agua', 0)

    doc.add_heading('Análisis preliminar - ' + report_data['sourceType'] + ' ' + report_data['sourceName'] + ', ' + report_data['zoneName'], 2)

    doc.add_paragraph('')

    doc.add_paragraph('Este reporte sobre el análisis preliminar de la calidad del agua es generado en la aplicación web del Sistema Roboat Stingray. Aquí se describen las características de las variables medidas y los datos estadísticos recolectados.')
    ...
    if report_data['sendTo'] != '':
        ...
        p.add_run(report_data['sendTo']) if report_data['sendTo'] else p.add_run('Aquí va la entidad o persona a quien se dirige el reporte')

        p.add_run('\n\n')

    p.add_run('Razón del reporte:\n').bold = True
    p.add_run(report_data['reason']) if report_data['reason'] else p.add_run( 'Aquí va la descripción del por qué se genera el reporte')

    p.add_run('\n\n')

    if report_data['descrip'] != '':
        p.add_run('Descripción del lugar:\n').bold = True
        p.add_run(report_data['descrip']) if report_data['descrip'] else p.add_run('Aquí va la descripción del lugar de estudio.')
    ...
    doc.add_paragraph('Hay que recordar que las sondas utilizadas son de marca AtlasScientific, cuyos rangos de medición son:')
    ...
    doc.add_paragraph('Se identificó que el ' + str(analysis['F']['% Cumplimiento Total'][1]) +
        '% del agua de la fuente hídrica, monitoreada en la ubicación ' + report_data['zoneName'] +
        ', cumple con las resoluciones ambientales: Resolución 2115 de 2007 y Objetivos de Calidad de Agua - Resolución 3560 de 2015; frente al '
        + str(analysis['F']['% Cumplimiento Total'][0]) + '% que no lo hace.')

    doc.add_paragraph('Es posible decir entonces que el ' + str(analysis['F']['% Cumplimiento Total'][1]) + '% del agua, tiene una alta probabilidad de ser potable.')

    doc.add_heading('Tabla de cumplimiento total:', 2)
    doc.add_paragraph('Esta tabla muestra el porcentaje de cumplimiento de ambas resoluciones sobre el total de tramos analizados, a partir del recorrido realizado por el sistema Roboat Stingray en la fuente hídrica.')
    ...
    doc.add_paragraph('Inserte aquí su captura de pantalla del recorrido realizado por el catamarán del sistema para añadir más información valiosa a si reporte.')
